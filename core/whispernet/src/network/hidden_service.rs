...
pub async fn launch_hidden_service(
    ...
    let (service, rend_requests) = tor_client.launch_onion_service(config)?.ok_or_else(|| {
        anyhow::anyhow!("Onion service already running or stream initialization failed")
    })?;
